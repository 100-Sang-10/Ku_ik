...
DynamicVehicleControl3::DynamicVehicleControl3() {
    waypoint_pub = nh.advertise<visualization_msgs::Marker>("/dynamic_vehicle_3/waypoint_marker", 100);
    control_pub = nh.advertise<carla_msgs::CarlaEgoVehicleControl>("/carla/dynamic_vehicle_3/vehicle_control_cmd", 1000);
    speed_sub = nh.subscribe("/carla/dynamic_vehicle_3/speedometer", 10, &DynamicVehicleControl3::SpeedCallback, this);
    odom_sub = nh.subscribe("/carla/dynamic_vehicle_3/odometry", 10, &DynamicVehicleControl3::PoseCallback, this);
    purepursuit_point_pub = nh.advertise<visualization_msgs::Marker>("/dynamic_vehicle_3/point_marker", 100);
}

DynamicVehicleControl3::~DynamicVehicleControl3() {}

void DynamicVehicleControl3::SpeedCallback(const std_msgs::Float32::ConstPtr speed_msg) {
    curr_speed = speed_msg->data;
}

void DynamicVehicleControl3::PoseCallback(const nav_msgs::Odometry::ConstPtr odom_msg) {
...
std::vector<double> DynamicVehicleControl3::WGS84toCartesian(double input_lat, double input_long) {
...
void DynamicVehicleControl3::waypoint() {
    // std::ifstream in("../Ku_ik/resources/dynamic_vehicle_3_waypoint.txt");
    std::ifstream in("../Ku_ik/src/control/dynamic_vehicle_control/resources/dynamic_vehicle_3_waypoint.txt");
...
std::vector<std::string> DynamicVehicleControl3::divide(std::string xy_str, char divider){
...
double DynamicVehicleControl3::to_num(std::string s){
...
void DynamicVehicleControl3::visualize_waypoint(){
...
double DynamicVehicleControl3::cal_yaw(const nav_msgs::Odometry::ConstPtr& odom_msg) {
    tf::Quaternion q(
...
std::pair<double, double> DynamicVehicleControl3::coordinate_tf(double input_x, double input_y) {
...
void DynamicVehicleControl3::next_point() {
...
void DynamicVehicleControl3::pure_pursuit(){
...
void DynamicVehicleControl3::purepursuit_next_point(){
...
void DynamicVehicleControl3::purepursuit_point_marker() {
...
void DynamicVehicleControl3::PID(){
...
void DynamicVehicleControl3::Control() {    
...
void DynamicVehicleControl3::Print() {
...
void DynamicVehicleControl3::Publish() {
    visualize_waypoint();
    purepursuit_point_marker();
    control_pub.publish(control_msg);
}

void DynamicVehicleControl3::Run() {
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "dynamic_vehicle_3_control");
    DynamicVehicleControl3 dynamic_vehicle_3_control;
    dynamic_vehicle_3_control.waypoint();
    ros::Rate loop_rate(30);

    while(ros::ok()) {   
        dynamic_vehicle_3_control.Run();
        ros::spinOnce();
        loop_rate.sleep();
    }
