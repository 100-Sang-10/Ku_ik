...
class Preprocessing{
    ...
    ros::Subscriber lidar_rear_sub;
    ros::Publisher rear_centroid_pub;

    pcl::PointCloud<pcl::PointXYZ> cloud;
    pcl::PointCloud<pcl::PointXYZ> cloud_rear;

public:
    Preprocessing();

    void pointcloud_front_cb(const sensor_msgs::PointCloud2ConstPtr& pointcloud_msg);
    void pointcloud_rear_cb(const sensor_msgs::PointCloud2ConstPtr& pointcloud_msg);
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> RightClustering( pcl::PointCloud<pcl::PointXYZ>::Ptr& inlierPoint_neg);
